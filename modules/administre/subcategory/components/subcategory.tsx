...
import {AddSubcategory} from '@modules/administre/subcategory/components/form';
import {Tablesubcategory} from '@modules/administre/subcategory/scenes/table';
...
export const Subcategory = () => {
  ...
  const metrics = useMemo(() => {
    ...
    const uniqueParent = new Set(dataSource.map(item => item.category)).size;
    const averageRating =
  ...
  const summaryCards = [
    {
      ...
      accent: 'from-emerald-500/50 to-teal-500/50 text-emerald-600',
    },
    {
      icon: HiOutlineAdjustmentsHorizontal,
      label: 'Subcategorías totales',
      value: metrics.total,
      accent: 'from-indigo-500/50 to-blue-500/50 text-indigo-600',
    },
    {
      icon: HiOutlineCursorArrowRipple,
      label: 'Categorias padre únicas',
      value: metrics.uniqueParent,
      accent: 'from-purple-500/50 to-pink-500/50 text-purple-600',
    },
  ];

  return (
    <section className='mx-auto flex w-full max-w-6xl flex-col gap-6 px-6'>
      <article className='relative isolate rounded-3xl border border-border/40 bg-gradient-to-br from-emerald-600 via-cyan-600 to-blue-600 px-8 py-12 text-white shadow-[0_25px_60px_-20px_rgba(59,130,246,0.55)] md:px-10'>
        <div
          ...
          <div className='max-w-2xl space-y-6 min-w-0'>
            ...
            <header className='space-y-4'>
              ...
              <p className='max-w-xl text-sm text-white/80 md:text-base'>
                Conecta colecciones con sus categorías principales, analiza la cobertura del
                catálogo y crea nuevas agrupaciones en segundos.
              </p>
            </header>
      ...
      <Modal title='Registrar subcategoría' open={openModal} onOpenChange={setopenModal}>
